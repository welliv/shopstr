...
describe("NostrNIP46Signer", () => {
  ...
  beforeEach(() => {
    ...
    (nip44.encrypt as jest.Mock).mockImplementation(
      (content) => `encrypted(${content})`
    );
    (nip44.decrypt as jest.Mock).mockImplementation((content) =>
      content.replace("encrypted(", "").replace(")", "")
    );
    (finalizeEvent as jest.Mock).mockImplementation((event) => ({
      ...event,
      id: "final-id",
      sig: "final-sig",
    }));
  ...
  describe("Serialization (toJSON/fromJSON)", () => {
    it("should correctly serialize and deserialize the signer", () => {
      const originalSigner = new NostrNIP46Signer(
        { bunker: validBunkerUrl, appPrivKey: mockAppPrivKey },
        mockChallengeHandler
      );
    ...
    it("should return undefined from fromJSON for invalid data", () => {
      const result = NostrNIP46Signer.fromJSON(
        { type: "invalid" },
        mockChallengeHandler
      );
